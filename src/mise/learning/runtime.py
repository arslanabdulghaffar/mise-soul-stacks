...
import numpy as np
from .preprocessing import preprocess_images, normalize_state, denormalize_actions, contextual_state
...
class TorchContactPolicy:
    def __init__(self, checkpoint: str | Path, *, device: str = 'cpu'):
        ...
        self.action_context = self.config.action_context
        self.model = ContactACT(self.config)
        self.model.load_state_dict(payload['model'])
        self.model.to(device).eval()

    def predict_chunk(self, observation, *, previous_action=None, elapsed_s=None):
        images = preprocess_images((observation.top, observation.wrist_a, observation.wrist_b), self.config.image_size)
        state = (contextual_state(observation.joint_positions, self.statistics, previous_action, elapsed_s)
                 if self.action_context else normalize_state(observation.joint_positions, self.statistics))
...
class OpenVINOContactPolicy:
    def __init__(self, directory: str | Path, *, device: str = 'CPU',
        ...
        self.action_context = self.metadata['config'].get('action_context', False)
        core = ov.Core()
