...
const LEARNED_COMMAND = 'Place the mug in the upper-right with arm B.'
const FIXTURE_COMMAND = 'Open the drawer with arm A.'
const controllerName = (controller?: string) => controller === 'learned_act' ? 'Learned ACT · mug' : controller === 'contact_expert' ? 'Contact expert' : controller === 'scripted_drawer' ? 'Drawer fixture (legacy)' : readable(controller)
...
export default function App() {
  ...
  const learnedAvailable = health?.capabilities?.controllers?.includes('learned_act') === true
  const controllerAvailable = controller === 'learned_act' ? learnedAvailable : controller === 'scripted_drawer' || contactAvailable
  const commandExecutable = controller === 'scripted_drawer' || previewExecutable === true && (controller !== 'learned_act' || planPreview.length === 1 && planPreview[0].object === 'mug' && planPreview[0].arm === 'B' && planPreview[0].target === 'table_upper_right')
  const supportedCommands = health?.capabilities?.supported_commands
  const commandExamples = controller === 'scripted_drawer' ? [FIXTURE_COMMAND] : Array.isArray(supportedCommands) ? supportedCommands : supportedCommands?.[controller] || []
  ...
  const isLearnedRun = shownController === 'learned_act'
  const isContactRun = isLearnedRun || shownController === 'contact_expert'
  ...
  const latestBenchmark = benchmarkRecords.at(-1)
  const intelTargetVerified = latestBenchmark?.intel_core_ultra_series_2_or_3_verified === true
  ...
  const cameraStage = <div className={`camera-card ${expanded ? 'expanded' : ''}`}>
    ...
    <div className="camera-bottom"><div className="camera-tabs" role="tablist" aria-label="Camera angle">{CAMERAS.map(cam => <button key={cam.id} role="tab" aria-selected={camera === cam.id} className={camera === cam.id ? 'selected' : ''} disabled={!!run && !active && cam.id !== 'top' && !run.artifacts?.[`video_${cam.id}`]} onClick={() => changeCamera(cam.id)}><span className={`camera-indicator ${cam.id === 'wrist_b' ? 'violet-dot' : ''}`} />{cam.label}</button>)}</div><span className="camera-size">{run?.camera_configuration ? `${run.camera_configuration.camera_dimensions?.[camera]?.width ?? run.camera_configuration.width}×${run.camera_configuration.camera_dimensions?.[camera]?.height ?? run.camera_configuration.height} · ${run.camera_configuration.camera_capture_hz[camera]} FPS (sim)` : replay ? 'Original recording' : 'RGB stream'}</span></div>
  </div>
  ...
  const planPanel = <aside className="panel plan-panel"><div className="panel-heading"><div><GitBranch size={16} /><h2>Execution plan</h2></div><Pill>{shownSteps.length} steps</Pill></div>
    ...
    <div className="plan-disclosure"><span className="tiny-dot" />{isLearnedRun ? 'Learned ACT · RGB, robot joints and action history.' : isContactRun ? 'Deterministic camera-guided IK/contact control.' : 'Legacy fixture · directly driven drawer.'}<br />{isFullRun ? 'Full-task outcome is scored by an isolated physics evaluator.' : isContactRun ? 'This skill remains separate from full-task scoring.' : 'No robot grasp or full-task result.'}</div>
  </aside>
    ...
    <div className="main-shell"><header className="topbar"><div className="breadcrumbs"><span>Workspace</span><ChevronRight size={14} /><strong>{view}</strong></div><div className="topbar-right"><span className="local-label"><Monitor size={14} />Local runtime</span><Pill dot tone={mode === 'LIVE' ? 'green' : mode === 'RECORDED REPLAY' ? 'violet' : 'muted'}>{mode}</Pill><span className="avatar" aria-label="SeoulStack">S</span></div></header>
      <main><div className="page-heading"><div><div className="eyebrow"><span className="heading-line" />MULTI-MODAL INSTRUCTION TO SKILL EXECUTION</div><h1>{view === 'Live run' ? 'The execution console' : view === 'Overview' ? 'A place for every action.' : view === 'Evidence' ? 'Show the whole story.' : view === 'Recovery' ? 'Every correction, accounted for.' : view === 'Benchmarks' ? 'Performance, with proof.' : 'From instruction to outcome.'}</h1><p>{view === 'Live run' ? isContactRun ? 'Run the complete camera-guided table setting task or a validated physical skill.' : 'Observe the legacy drawer fixture and its recorded outcome.' : metadata.detail}</p></div><div className="heading-aside">{view === 'Live run' ? <><span className="round-icon"><Activity size={18} /></span><div><strong>{active ? readable(run?.status) : replay ? 'Recorded execution' : operatorEnabled ? 'Ready for live execution' : connected ? 'Replay mode' : 'Runtime disconnected'}</strong><span>{run ? `Seed ${run.seed} · ${shortId(run.id)}` : controllerName(controller)}</span></div></> : <Pill>RESEARCH BUILD · 01</Pill>}</div></div>
        ...
        {view === 'Live run' && <>
          ...
          <div className="run-options"><div><span className="muted">Controller</span><label className="select-wrap controller-select"><Cpu size={13} /><select value={controller} disabled={active || !operatorEnabled} aria-label="Execution controller" onChange={event => { const value = event.target.value as Controller; setController(value); setCommand(value === 'scripted_drawer' ? FIXTURE_COMMAND : value === 'learned_act' ? LEARNED_COMMAND : DEFAULT_COMMAND); if (value === 'learned_act') setPreset('nominal'); setError('') }}><option value="contact_expert" disabled={!contactAvailable}>Contact expert{!contactAvailable ? ' · unavailable' : ''}</option><option value="learned_act" disabled={!learnedAvailable}>Learned ACT · mug only{!learnedAvailable ? ' · install model' : ''}</option><option value="scripted_drawer">Drawer fixture (legacy)</option></select><ChevronDown size={13} /></label><span className="options-separator" /><span className="muted">Scenario</span><label className="select-wrap"><Settings2 size={13} /><select value={preset} onChange={e => setPreset(e.target.value)} disabled={active || !operatorEnabled || controller === 'learned_act'} aria-label="Reproducible perturbation preset"><option value="nominal">Nominal scene</option><option value="low_friction">Reduced friction · stress preset</option><option value="displaced_objects">Displaced objects · stress preset</option></select><ChevronDown size={13} /></label><span className="options-separator" /><span className="muted">Recovery</span><label className="select-wrap"><GitBranch size={13} /><select value={recoveryMode} onChange={e => setRecoveryMode(e.target.value as RecoveryMode)} disabled={active || !operatorEnabled || controller !== 'contact_expert'} aria-label="Recovery policy"><option value="adaptive">Monitored · cost selected</option><option value="blind_retry">Bounded blind retry</option><option value="none">No recovery</option></select><ChevronDown size={13} /></label></div><span className="run-id"><Fingerprint size={13} />{shortId(run?.id)}</span></div>
          <div className="view-options"><label htmlFor="view-quality">Camera quality</label><select id="view-quality" value={viewQuality} onChange={e => setViewQuality(e.target.value)} disabled={active || !operatorEnabled}><option value="economy">Fast live · 256×256</option><option value="balanced">Balanced · 384×384 · smoother motion</option><option value="detail">Detail · 720×720 · sharper, slower capture</option></select><span>All views update together. Smaller close-up frames keep Fast live quicker. Applies to the next run.</span></div>
